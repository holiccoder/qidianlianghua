...
const LOG_RPC_URLS = ["https://bsc.drpc.org", "https://bsc-rpc.publicnode.com"];
...
function normalizeEvmAddress(value?: string): string {
  ...
  return normalized && /^0x[a-fA-F0-9]{40}$/.test(normalized) ? normalized : "";
}
...
export function useWalletInfo() {
  ...
  const fetchBalance = useCallback(async () => {
    try {
      const result = await balanceRpcCall("eth_getBalance", [
        WALLET_ADDRESS,
        "latest",
      ]);
...
export function useBurnRecords() {
  ...
  const fetchBurnRecords = useCallback(async () => {
    try {
      ...
      const configPayload =
        (await configResponse.json()) as PublicSettingsResponse;
      const contractAddress = normalizeEvmAddress(
        configPayload.tokenContractAddress
      );
      ...
      const currentBlockHex = (await balanceRpcCall(
        "eth_blockNumber",
        []
      )) as string;
          ...
          const logs = (await logRpcCall("eth_getLogs", [
            {
              ...
              topics: [TRANSFER_TOPIC, walletPadded, deadPadded],
            },
          ])) as Array<{
            transactionHash: string;
            blockNumber: string;
      ...
      const uniqueBlockNums = Array.from(
      ...
      for (let i = 0; i < uniqueBlockNums.length; i += batchTimestampSize) {
        ...
        await Promise.allSettled(
          batch.map(async blockNum => {
            try {
              const block = (await logRpcCall("eth_getBlockByNumber", [
                blockNum,
                false,
              ])) as {
                timestamp: string;
              ...
              const approxTimestamp =
                Math.floor(Date.now() / 1000) - blockDiff * 3;
